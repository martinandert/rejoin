...
function mergeSettingsIntoModel(model, spec) {
  if (typeof spec.table !== 'undefined') {
    model.setTableName(spec.table);
  }

  if (typeof spec.abstract !== 'undefined') {
    model.setAbstractModel(spec.abstract);
  }
}
...
var ModelCreator = {
  createModel: function(name, spec) {
    ...
    var extended = spec.extends || ModelCreator.getRejoinModel();
    var newModel = createModelAsExtensionOf(extended, name);

    mergeSettingsIntoModel(newModel, spec);
    mergeSpecIntoModel(newModel, spec);

    return newModel;
  },

  getRejoinModel: function() {
    if (typeof ModelCreator._rejoinModel === 'undefined') {
      ModelCreator._rejoinModel = require('./Model');
    }

    return ModelCreator._rejoinModel;
  }
};
