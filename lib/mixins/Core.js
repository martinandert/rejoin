...
var Callback = require('../core/Callback').Name;
...
module.exports = {
  ...
  prototype: {
    initialize: function(attributes, cb) {
      ...
      this.runCallbacks(Callback.INITIALIZE, this._super, arguments);
    },

    initWith: function(coder, cb) {
      ...
      this.runCallbacks(Callback.FIND, function(err, result) {
        if (err) {
          cb(err);
        } else {
          result.runCallbacks(Callback.INITIALIZE, function(err, result) {
            if (err) {
              cb(err);
            } else {
              cb(null, this);
