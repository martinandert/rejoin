...
function PersistenceMixin(model) {
  model.classMethods({
    instantiate: function(record, cb) {
      var Model = this._discriminateModelForRecord(record);
      var instance = new Model();
      instance.populate({ attributes: record }, cb);
    },
  ...
  model.instanceMethods({
    ...
    touch: function() {
      var attributeNames = sliced(arguments);
      var cb = attributeNames.pop();
      ...
      if (isPresent(attributeNames)) {
        attributes = attributes.concat(attributeNames);
      }
      ...
      this._changedAttributes = _.omit(this._changedAttributes, Object.keys(changes));
    ...
    _createRecord: function(attributeNames, cb) {
      if (_.isFunction(attributeNames)) {
        cb = attributeNames;
        attributeNames = null;
      }
