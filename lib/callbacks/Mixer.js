...
var Mixer = {
  mixInto: function(ctorFunc, copyFromCtorFunc) {
    ...
    ctorFunc.defineCallbackChains = function() {
      ...
      if (_.isPlainObject(_.last(names))) {
    ...
    ctorFunc.registerCallback = function() {
      ...
      if (_.isPlainObject(_.last(filters))) {
        extend(options, filters.pop());
      }

      if (_.has(chain.getConfig(), 'optionsTransformer')) {
        chain.getConfig().optionsTransformer(options);
      }
      ...
      if (chain.isEmpty()) {
        ...
        chain.compile(function(run) {
          ...
          run(env, function(env) {
            done.call(env.target, env.error, _.has(env, 'result') ? env.result : env.halted ? false : env.target);
          });
        });
      }
    };
