...
var StrictValidationFailed = require('../Errors').StrictValidationFailed;
...
Errors.prototype.clone = function() {
  var cloned = new Errors(this.record);
  cloned.messages = _.clone(this.messages);
  return cloned;
};
...
Errors.prototype.includes = function(key) {
...
Errors.prototype.isEmpty = function() {
  return _.every(this.getValues(), _.isEmpty);
};

Errors.prototype.isBlank = function() {
  return this.isEmpty();
};

Errors.prototype.asJSON = function(options) {
  options = options || {};
  _.defaults(options, { fullMessages: false });

  return this.toHash(options.fullMessages);
};
...
Errors.prototype.add = function(key, error, options) {
  ...
  var Exception = options.strict;

  if (Exception) {
    if (Exception === true) {
      Exception = StrictValidationFailed;
    }

    throw new Exception(this.getFullMessage(key, error));
  }
